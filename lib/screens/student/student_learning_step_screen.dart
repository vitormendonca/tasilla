...
class _StudentLearningStepScreenState extends State<StudentLearningStepScreen> {
  ...
  Future<void> _completeStep() async {
    ...
    if (!_allSectionsAnswered) {
      setState(() {
        _completionMessage =
            'Answer all questions before completing this step.';
      });
      return;
    }
  ...
  void _resetAttempt() {
    setState(() {
      ...
      _listeningQuestionsAnswered = false;
            ...
            child: Column(
              ...
              children: [
                Container(
                  ...
                  height: 6,
                  decoration: BoxDecoration(
                    color: _stepColor(widget.step.type),
                ...
                child: isSaving
                    ...
                    : Text(
                        _attemptedAndFailed
                            ...
                            ? 'ALREADY COMPLETED'
                            : _completionLabel(widget.step.type).toUpperCase(),
                        style: TextStyle(
                          fontSize: 11,
                          fontWeight: FontWeight.w700,
                          letterSpacing: 1.0,
                          color: _attemptedAndFailed || !isDark
                              ? Colors.white
                              : const Color(0xFF161618),
                        ),
                      ),
  ...
  Widget _vocabularySection(
    List<VocabularyBlock> blocks, {
    required Color textPrimary,
    required Color textMuted,
  }) {
    return Column(
      ...
      children: [
        ...
        for (final block in blocks) ...[
          ...
          Wrap(
            ...
            runSpacing: 7,
            children: [
              for (final word in block.words) _contentChip(word, textMuted),
            ],
          ),
        ...
        for (final block in blocks) ...[
          ...
          if (block.patterns.isNotEmpty) ...[
            ...
            Wrap(
              ...
              runSpacing: 7,
              children: [
                for (final p in block.patterns) _contentChip(p, textMuted),
              ],
            ),
          ],
        ],
  ...
  Widget _listeningSection(
    ...
    return Column(
      ...
      children: [
        ...
        const SizedBox(height: 7),
        if (block.hasPlannedAudioPath)
          LessonAudioPlayer(
            key: ValueKey('lesson_audio_$_attemptNumber'),
            audioPath: block.audioPath!,
            transcript: block.audioScript,
            maxPlays: block.maxAudioPlays,
            allowTranscript: _listeningQuestionsAnswered,
          )
        else
          Text(
            block.audioScript,
        ...
        if (block.listeningQuestions.isNotEmpty) ...[
          ...
          InteractiveQuizSection(
            ...
            onResultChanged: (result) {
              ...
              if (result.allAnswered != _listeningQuestionsAnswered &&
                  mounted) {
                setState(() {
                  _listeningQuestionsAnswered = result.allAnswered;
                });
              }
            },
          ),
  ...
  Widget _writingSection(
    ...
    return _taskSection(
      ...
      details: [
        '${task.minSentences}-${task.maxSentences} sentences',
        if (task.requiresTeacherReview) 'Teacher review',
      ],
      requirements: task.minimumRequirements,
  ...
  Widget _speakingSection(
    ...
    return _taskSection(
      ...
      details: [
        '${task.maxRecordingSeconds}s max',
        if (task.requiresTeacherReview) 'Teacher review',
      ],
      requirements: const [],
      color: textMuted,
      ...
      children: [
        ...
        if (details.isNotEmpty) ...[
          ...
          Wrap(
            spacing: 7,
            runSpacing: 7,
            children: [for (final d in details) _contentChip(d, color)],
          ),
        ],
  ...
  Widget _guidedPracticeFallback({
    required Color textMuted,
    required Color textPrimary,
    required Color border,
  }) {
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          Text(
            'Complete the prompt above aloud or in writing. This practice has no auto-graded questions, so review your answer before marking it complete.',
            style: TextStyle(fontSize: 12, color: textMuted, height: 1.35),
          ),
        ],
      ),
  ...
  String _exercisePrompt(LearningPathStep step) {
    if (step.skillId == a1RoadmapSkillId) {
      switch (step.type) {
        case LearningPathStepType.lesson:
          return 'This core experience represents one focused block inside the guided A1 certificate track.';
        case LearningPathStepType.reinforcement:
          return 'This smart reinforcement strengthens recent A1 language before the student moves forward.';
        case LearningPathStepType.review:
          return 'This mixed review combines recent A1 skills before the student moves forward.';
        case LearningPathStepType.checkpoint:
          return 'This checkpoint helps the teacher confirm readiness and recommend review if needed.';
        case LearningPathStepType.portfolio:
          return 'This portfolio task stores writing or speaking evidence for certificate readiness.';
        case LearningPathStepType.finalTest:
          return 'This final exam covers the complete A1 track and should feel stricter than normal practice.';
      }
    }
    switch (step.type) {
      case LearningPathStepType.lesson:
        return 'This lesson represents one focused practice block in the ${step.skillTitle} A1 path.';
      case LearningPathStepType.reinforcement:
        return 'This reinforcement should revisit recent language with short targeted practice.';
      case LearningPathStepType.review:
        return 'This review should mix the previous three lessons before the student moves forward.';
      case LearningPathStepType.checkpoint:
        return 'This checkpoint should confirm whether the student is ready to continue.';
      case LearningPathStepType.portfolio:
        return 'This portfolio task should collect certificate evidence for teacher review.';
      case LearningPathStepType.finalTest:
        return 'This test should be stricter than normal lessons and count toward skill completion.';
    }
  }
