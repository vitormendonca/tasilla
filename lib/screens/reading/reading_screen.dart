...
class _ReadingScreenState extends State<ReadingScreen> {
  ...
  Future<void> openActivity(ReadingActivity activity) async {
    await Navigator.push(context, MaterialPageRoute(builder: (_) => ReadingActivityScreen(activity: activity)));
    await loadProgress();
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Reading Practice', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: textPrimary)),
      ),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          Text('READING PRACTICE', style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, letterSpacing: 1.6, color: textMuted)),
          const SizedBox(height: 6),
          Text('Choose a reading practice', style: TextStyle(fontSize: 24, fontWeight: FontWeight.w300, color: textPrimary, letterSpacing: -0.5)),
          const SizedBox(height: 6),
          Text('Read short texts, answer questions, and check your comprehension.', style: TextStyle(fontSize: 13, color: textMuted, height: 1.4)),
  ...
  Widget _readingActivityCard({
    ...
    return GestureDetector(
      ...
      child: Container(
        ...
        child: Row(
          children: [
            Container(width: 6, height: 6, decoration: BoxDecoration(color: isCompleted ? AppTheme.semanticGreen : textMuted, shape: BoxShape.circle)),
            ...
            Expanded(
              child: Column(
                ...
                children: [
                  ...
                  Text(activity.description, style: TextStyle(fontSize: 12, color: textMuted, height: 1.35)),
                  const SizedBox(height: 6),
                  Row(
                    children: [
            ...
            Container(
              ...
              child: Text(activity.level, style: TextStyle(fontSize: 9, fontWeight: FontWeight.w600, color: textMuted)),
            ),
          ],
        ),
      ),
...
class _ReadingActivityScreenState extends State<ReadingActivityScreen> {
  ...
  int _availableQuestionsTotal() => widget.activity.questions.where(_isQuestionAvailable).length;
  ...
  int _calculateScore() {
    ...
    for (final question in widget.activity.questions) {
      if (!_isQuestionAvailable(question)) continue;
      final answer = selectedAnswers[question.id];
      if (answer != null && _isCorrect(question, answer)) score++;
    }
    return score;
  }
  ...
  Future<void> _saveResult() async {
    ...
    if (percentageScore >= 85) {
      await StudentProgressService.markActivityAsCompleted(activityId: widget.activity.id, category: 'reading');
      if (currentStudentName.isNotEmpty) {
        await AssignmentService.markStudentAssignmentAsCompleted(studentName: currentStudentName, title: widget.activity.title, category: 'Reading');
      }
    } else {
      if (currentStudentName.isNotEmpty) {
        await AssignmentService.markStudentAssignmentAsReviewNeeded(studentName: currentStudentName, title: widget.activity.title, category: 'Reading');
      }
    }

    await StudentProgressService.saveActivityScore(activityId: widget.activity.id, category: 'reading', score: percentageScore);
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        scrolledUnderElevation: 0,
        iconTheme: IconThemeData(color: textMuted),
        title: Text(activity.title, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: textPrimary)),
      ),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          ...
          Text(activity.title, style: TextStyle(fontSize: 22, fontWeight: FontWeight.w300, color: textPrimary, letterSpacing: -0.5)),
          const SizedBox(height: 4),
          Text(activity.level, style: TextStyle(fontSize: 12, color: textMuted)),
          ...
          Container(
            ...
            child: Text(activity.text, style: TextStyle(fontSize: 14, color: textMuted, height: 1.55)),
          ),
          ...
          GestureDetector(
            onTap: _allQuestionsAnswered() ? _finishReading : null,
            child: Container(
              height: 52,
              width: double.infinity,
              decoration: BoxDecoration(
  ...
  Widget _buildAnswerOption({
    ...
    return GestureDetector(
      onTap: hasAnswered ? null : () { setState(() { selectedAnswers[question.id] = option; }); },
      child: Container(
        width: double.infinity,
        ...
        child: Row(
          children: [
            ...
            if (hasAnswered && isCorrectAnswer) Icon(Icons.check_circle_outline, color: AppTheme.semanticGreen, size: 18),
            if (isWrongSelected) Icon(Icons.cancel_outlined, color: AppTheme.semanticRed, size: 18),
          ],
        ),
      ),
    );
      ...
      child: Column(
        ...
        children: [
          ...
          TextField(
            ...
            onChanged: (value) { setState(() { selectedAnswers[question.id] = value; }); },
          ),
          if (hasAnswered) ...[
            const SizedBox(height: 10),
  ...
  Widget _buildResultScreen() {
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        scrolledUnderElevation: 0,
        iconTheme: IconThemeData(color: textMuted),
        title: Text(reviewMode ? 'Reading Review' : 'Reading Result', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: textPrimary)),
      ),
      body: ListView(
        padding: const EdgeInsets.all(20),
        children: [
          Container(
            ...
            child: Column(
              children: [
                Icon(isApproved ? Icons.check_circle_outline : Icons.refresh, color: resultColor, size: 36),
                const SizedBox(height: 12),
                Container(
                  padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 3),
                  decoration: BoxDecoration(
          ...
          for (int i = 0; i < widget.activity.questions.length; i++)
            _buildReviewCard(index: i, question: widget.activity.questions[i], textPrimary: textPrimary, textMuted: textMuted, surface: surface, border: border),
          ...
          GestureDetector(
            ...
            child: Container(
              ...
              child: Center(child: Text('Back to Reading', style: TextStyle(fontSize: 14, fontWeight: FontWeight.w500, color: textPrimary))),
            ),
          ),
          const SizedBox(height: 20),
        ],
